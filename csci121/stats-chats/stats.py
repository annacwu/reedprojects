...
def wordFrequencies(wordList):
    ...
    # scans the list of words
    freqs = {}
    for i in wordList:
        # if the word is not a '.', '!', or '?'
...
def wordCount(freqs):
    ...
    # for every key in the frequencies dictionary
    count = 0
    vocabSize = 0
    for i in freqs:
        # increment count by the value for that key
        count += freqs[i]
        # increment the number of distinct words by one
        vocabSize += 1
...
def topWordExcept(freqs,excluded):
    ...
    big = 0
    top = None
    # for every item in the frequencies dictionary
    for i in freqs:
        if i not in excluded:
            # if the value of that word is greater than the greatest to be previously stored
            if freqs[i] > big:
                # it is now the greatest to be stored
                big = freqs[i]
                # the top word is the word associated with that value
                top = i
...
def topWordsByFrequency(freqs,howMany):
    ...
    topWords = []
    seen = []
    # i variable counts the index with each iteration of the while loop
    i = 0
    # while the length of the topWords list is less than the number we want
    while len(topWords) < howMany:
        # add the next top word from the list to topWords
        topWords.append(topWordExcept(freqs,seen))
        # add that word to the exception list
        seen.append(topWords[i])
        i += 1
...
def numWordsWithFrequency(freqDict,freq):
    ...
    # for every word in the frequency dictionary
    num = 0
    for i in freqDict: 
        # if it appears the specfied number of times
        if freqDict[i] == freq:
